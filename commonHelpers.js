          ...
          <div class="cart-item-info-wrap">
            ...
            <div class="cart-item-descr">
              ...
              <div class="cart-text-wrap">
                ...
                <p class="cart-item-text">
                  <span class="cart-light-text">Size:</span>&nbsp;&nbsp;${s}
                </p>
              </div>
              <p class="cart-item-price">$${m}</p>
            </div>
          </div>
        </li>`}).join("");return e.cartProductList.innerHTML=r}e.deleteAllBtn.addEventListener("click",f);function f(){e.cartProductList.innerHTML="",p(),u(),l(),e.cartMainContainer.hidden=!0,e.emptyBasketContent.hidden=!1,e.emptyBasketWrap.style.display="block"}function u(){const r=c().reduce((d,{price:n,quantity:o})=>d+=n*o,0);e.cartTotalPrice.textContent=r.toFixed(2),y()}async function B(t){t.preventDefault();const r=t.currentTarget.elements.email.value;if(!r){alert("Please, write your email!");return}P(r),t.currentTarget.reset();const n=c().map(({_id:a})=>({productId:a,amount:1})),o={email:r,products:n};try{let a=function(){document.getElementById("my-modal").classList.add("open")};document.getElementById("open-modal-btn").addEventListener("click",a);const s=await fetch("https://food-boutique.b.goit.study/api/orders",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)});if(s.ok)a(),p(),f(),y();else if(s.status===400){const i=await s.json();alert("Bad request: "+i.message)}else if(s.status===404)alert("Resource not found.");else if(s.status===500)alert("Server error. Please try again later.");else throw new Error("Failed to subscribe.")}catch(a){alert("Error: "+a.message)}}function P(t){if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(t)){alert("Please, enter valid Email!");return}}k.addEventListener("click",function(){v.classList.remove("open")});function q(){c()}
